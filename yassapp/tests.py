...
class CreateAuctionTestCase(TestCase):
    ...
    # asserts that the title of the mail is correct
    def test_mail_title(self):
        self.auction_save()
        self.assertEquals(mail.outbox[0].subject, 'New Auction Created')

    # asserts that the body of the mail is correct
    def test_mail_message(self):
        self.auction_save()
        self.assertEquals(mail.outbox[0].body, 'Your New Auction has been created, here is the link: http://127.0.0.1:8000/auction/' + str(
                self.auction.id) + '/')

    # asserts that the sender of the mail is correct
    def test_mail_from(self):
        self.auction_save()
        self.assertEquals(mail.outbox[0].from_email, 'noreply@yaas.com')

    # asserts that the recipient of the mail is correct
    def test_mail_recipient(self):
        self.auction_save()
        self.assertEquals(mail.outbox[0].to, [self.owner.email])

    # asserts that a deadline less than three days layer from now is invalid
    def test_deadline_min_invalid(self):
        form_data = {
            'title': self.title,
    ...
    # is valid
    def test_deadline_min_valid(self):
        form_data = {
            'title': self.title,
            'description': self.description,
            'price': self.price,
    ...
    def test_auction_discarded(self):
    ...
    def test_auction_published(self):
        before = Auction.objects.all().count()
        response = self.new_auction_request({
            'a_title': self.title,
            'a_description': self.description,
    ...
    def test_auction_published_correctly(self):
...
class BidAuctionTestCase(TestCase):
    ...
    def test_bid_bidder_is_seller(self):
    ...
    def test_bid_lower_than_base_price(self):
        response = self.new_bid_request({
            'price': 19.10
        })
